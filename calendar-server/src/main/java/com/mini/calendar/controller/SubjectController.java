...
public class SubjectController {
    ...
    public BaseResponse saveSubjectComment(@RequestBody SubjectCommentRequest request){
        if (StringUtils.isEmpty(request.getSpaceId())
                || StringUtils.isEmpty(request.getSubjectId())
                || StringUtils.isEmpty(request.getUserId())){
            return BaseResponse.fail("01", "参数异常");
        }
        subjectService.saveSubjectComment(request);
        return BaseResponse.success("success");
    }
    ...
    public BaseResponse saveSubjectPraise(@RequestBody SubjectPraiseRequest request){
        if (StringUtils.isEmpty(request.getSpaceId())
                || StringUtils.isEmpty(request.getSubjectId())
                || StringUtils.isEmpty(request.getUserId())){
            return BaseResponse.fail("01", "参数异常");
        }
        subjectService.saveSubjectPraise(request);
        return BaseResponse.success("success");
    }
